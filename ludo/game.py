...
import random
from ludo.painter import PaintBoard
...
class Board():
    ...
    def is_pawn_on_board_pool(self, pawn):
        return self.pawns_position[pawn] == self.board_pool_position
    ...
    def can_pawn_move(self, pawn, rolled_value):
        common_poss, private_poss = self.pawns_position[pawn]
    ...
    def get_pawns_on_same_position(self, pawn):
        position = self.pawns_position[pawn]
        return [curr_pawn for curr_pawn, curr_pos in self.pawns_position.items() if position == curr_pos]
...
class Game():
    ...
    def get_allowed_pawns_to_move(self, player, rolled_value):
        ...
        for pawn in player.pawns:
            if not self.board.is_pawn_on_board_pool(pawn) and self.board.can_pawn_move(pawn, rolled_value):
                allowed_pawns.append(pawn)
    ...
    def _jog_foreign_pawns(self, pawn):
        pawns = self.board.get_pawns_on_same_position(pawn)
        for p in pawns:
    ...
    def _make_move(self, player, pawn):
        if self.rolled_value == Die.MAX and self.board.is_pawn_on_board_pool(pawn):
            self.board.put_pawn_on_starting_square(pawn)
            self._jog_foreign_pawns(pawn)
            return
    ...
    def play_turn(self, ind=None, rolled_val=None):
        ...
        if self.allowed_pawns:
            if ind is None:
                self.index = self.curr_player.choose_pawn(self.allowed_pawns)
            else:
                self.index = ind
