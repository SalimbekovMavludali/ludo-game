...
class CLIGame():
    ...
    def prompt_for_player(self):
        ...
        if choice == 1:
            name = self.validate_input("Enter name for player", str, str_len=(1, 20))
            available_options = range(len(available_colours))
            if len(available_options) > 1:
                options = ["{} - {}".format(index, colour)
                           for index, colour in zip(available_options, available_colours)]
    ...
    def print_players_info(self):
        ...
        print("Game {} with {} players".format(word, len(self.game.players)))
    ...
    def print_info_after_turn(self):
        ...
        if self.game.allowed_pawns:
            message_moved = "{} is moved. ".format(self.game.picked_pawn.id)
    ...
    def continue_recorded_game(self):
        ...
        for rolled_value, index in self.record_runner:
            self.game.play_turn(index, rolled_value)
            self.record_maker.add_game_turn(self.game.rolled_value, self.game.index)
        self.print_players_info()
        self.print_info_after_turn()
        self.print_board()
    ...
    def load_recorded_players(self):
        if self.recorded_runner is None:
    ...
    def play_game(self):
        try:
            ...
            print(linesep + "Exiting game..." + "Save game and continue same game later?")
            self.offer_save_game()
            raise
